...
train <- fread(file.path(data_dir, "TRAIN.csv.csv"))
test <- fread(file.path(data_dir, "TEST_FINAL.csv.csv"))
sample_sub <- fread(file.path(data_dir, "SAMPLE.csv.csv"))
...
server <- function(input, output) {
  ...
  output$forecast_plot <- renderPlot({
    ggplot(filtered_data(), aes(x = Date, y = Sales)) +
      ...
      labs(
        title = paste("Forecasted Sales for", input$region),
        y = "Sales",
        x = "Date"
      )
  })
